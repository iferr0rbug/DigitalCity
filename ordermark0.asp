...
<%
set rs_ordermark0=server.createobject("adodb.recordset")
		...
		<div class="col-md-9">
			<div class="specia-top">
                   <%
                		...
                		for i = 1 to rs_ordermark0.RecordCount 
               		%>
                    ...
                    <tr>
                        <td>
                            <table class="table table-hover">
                                <tbody>
                                    <tr>
                                        <td>订单号：<%=rs_ordermark0("订单ID")%></td>
                                        <td>日期：<%=rs_ordermark0("订单日期")%></td>
                                        <td>处理状态：<%=rs_ordermark0("处理结果")%></td>
                                        <td><a href="markorder.asp?mark_orderID=<%=rs_ordermark0("订单ID")%>&mark=1">标记为已处理</a></td>
                                        <td><a href="delorder.asp?del_orderID=<%=rs_ordermark0("订单ID")%>" onClick="return confirm('真的要删除这份订单吗？')">删除</a></td>
                                	</tr>
                                </tbody>
                            </table>
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    <% for j = 1 to rs_orderlist.RecordCount 
										...
										if CStr(rs_orderlist("订单ID")) = CStr(rs_ordermark0("订单ID")) then
                            		%>
                                    <tr>
